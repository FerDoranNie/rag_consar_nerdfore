...
def main_pipeline(folder_path: str, 
    ...
    if save_type=='csv':
        ...
        df_projects, df_work_type, df_clients = process_iris_excel_file(excel_file,
                                                                        sheet_names = sheet_names)

        all_results_df = pd.merge(
            all_results_df, 
            df_projects,
            left_on = ['filename_siglas_proyecto'], 
            right_on = ['siglas_proyecto'],
            how='left')
        ...
        all_results_df = pd.merge(
            all_results_df, 
            df_work_type, 
            left_on=['filename_tipo_trabajo'], 
            right_on=['siglas_tipo_trabajo'],
            how='left')
...
def pipeline_texts_to_json(input_dir, 
    ...
    project_siglas = df_projects['siglas_proyecto'].unique()
    all_chunks = []
    ...
    for filename, text in file_texts.items():
        try:
            ...
            # 2. Enriquecer los metadatos con información de los DataFrames de configuración
            # Buscar el tipo de trabajo
            work_type_info = df_work_type[df_work_type['siglas_tipo_trabajo'] == filename_info.get('tipo_trabajo')]
            if not work_type_info.empty:
                filename_info.update(work_type_info.iloc[0].to_dict())

            # Buscar el cliente y su categoría
            project_info = df_projects[df_projects['siglas_proyecto'] == filename_info.get('siglas_proyecto')]
            if not project_info.empty:
                client_name = project_info.iloc[0]['cliente']
                client_info = df_clients[df_clients['cliente'] == client_name]
                if not client_info.empty:
                    filename_info.update(client_info.iloc[0].to_dict())
